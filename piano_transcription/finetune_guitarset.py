...
def finetune_guitarset(args):
    ...
    # Paths
    hdf5s_dir = os.path.join(workspace, 'hdf5s', 'guitarset', audio_type)
        ...
        if iteration % 1000 == 0 and iteration > 0:
            ...
            if validate_statistics:
                ...
                if performance_metric is not None and performance_metric > best_validation_metric:
                    ...
                    best_checkpoint_path = os.path.join(log_checkpoints_dir, f'best_model_log{current_log_number}.pth')
                    torch.save({
                        'iteration': iteration,
                        'model': model.state_dict(),
                        'optimizer': optimizer.state_dict(),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--audio_type', type=str, default='combined',
        choices=['combined', 'audio_hex_debleeded', 'audio_hex_original', 'audio_mix', 'audio_mono-mic'],
        help='Type of GuitarSet audio to use. "combined" uses all audio types (4x more data!)')
